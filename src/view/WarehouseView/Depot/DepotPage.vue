<template>
      ...
      <div class="table-content">
        <div class="table-function sticky">
          ...
          <div class="table-function_search">
            ...
            ></div>
            <a target="_blank" :href="configs.baseUrl + '/Depots/export_data?keyword=' + keyword" class="action-render_table export-data"></a>
          </div>
        </div>
  ...
  export default {
    ...
    setup() {
      const { getRecordList, getRecordApi, deleteRecordApi, toggleActiveApi } = apiDepot;
      /**
       ...
       * Khắc Tiềm - 15.09.2022
       */
      const columns = computed(() => store.state.depot.columns.filter(function (value) { return value.isShow; }));
       ...
       * Khắc Tiềm - 15.09.2022
       */
      const recordList = computed(() => store.state.depot.recordList);
  
      /**
       * Lấy ra tổng số lượng bản ghi
       * Khắc Tiềm - 15.09.2022
       */
      const totalCount = computed(() => store.state.depot.totalCount);
  
      /**
       * Lấy danh sách các chức năng
       * Khắc Tiềm - 15.09.2022
       */
      const actionTable = computed(() => store.state.depot.actionTable);
      ...
      async function loadData(filter) {
        if (filter) {
          await store.dispatch("depot/setFilterAction", filter);
          //Kích hoạt hiệu ứng loader table
          isShowLoaderTable.value = true;
        }
        const find = store.state.depot.filter;
        await callApi(getRecordList, find, (response) => { store.dispatch("depot/getRecordListAction", response); }, store, true);
        if (filter) {
      ...
      async function toggleRecordActiveApi(recordId){
        await callApi(toggleActiveApi, recordId, () => { 
          store.dispatch("depot/setToggleActiveAction", recordId);
         }, store, true);
      }
