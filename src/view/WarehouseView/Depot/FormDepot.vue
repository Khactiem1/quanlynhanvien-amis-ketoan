<template>
      ...
      <div class="form">
        ...
        <div class="form-container">
          <div class="form-item_input">
            ...
            <input-default
              ...
              :type="'text'"
              :maxLength="20"
              :messageValid="depotNotification.validateCode"
              :label="'Mã'"
            ...
            <div class="form-group ms-big">
              <input-default
                ...
                :type="'text'"
                :maxLength="80"
                :messageValid="depotNotification.validateName"
                :label="'Tên'"
        ...
        <div class="form-action">
          <div class="form-action_container">
            ...
            <div class="form-action_item">
              <button @click="handleCloseModal(true)" :tabindex="6" class="btn">
                Huỷ
              </button>
            </div>
          </div>
  ...
  export default {
    ...
    setup(props, context) {
      const { createRecordApi, editRecordApi } = apiDepot;
      /**
       ...
       * NK Tiềm 28/10/2022
       */
      const { QUESTION_DATA_CHANGE, depotNotification, } = notificationMessage;
       ...
       * NK Tiềm 28/10/2022
       */
      const { TypeSuccess, MessageSuccessAdd, MessageSuccessEdit, AddFormDepot, EditFormDepot, } = eNum;
       ...
       */
      onBeforeMount(async () => {
        titleForm.value = AddFormDepot;
        if (recordEdit.value) {
          titleForm.value = EditFormDepot;
          depot.value = { ...recordEdit.value }; 
          stateAdd.value = false;
          recordEditReset.value = { ...recordEdit.value };
        } else {
          titleForm.value = AddFormDepot;
          if (recordAdd.value) {
            depot.value = { ...recordAdd.value };
          }
        }
      });
      ...
      function validateInput() {
        return [
          // Validate code
          depot.value.depotCode.trim() === "" ? depotNotification.validateCode : null,
          depot.value.depotName.trim() === "" ? depotNotification.validateName : null,
        ].filter((item) => {
          if(item){
            return true;
          }
        ...
        await callApi(Api, { ...depot.value },async (response) => { 
          ...
          store.dispatch("config/addNotification", {
            ...
            if(!stateAdd.value){
              ...
              stateAdd.value = true; 
              store.dispatch("depot/editRecordAction", { ...depot.value, depotID: response });
            }
            else{
              store.dispatch("depot/addRecordAction", { ...depot.value, depotID: response });
            }
         }, store, false, () => {
          errorApi.value = true;
         });
      ...
      function handleCloseModal(closeNow) {
        ...
        try {
          if (closeNow) {
            ...
            if (
              JSON.stringify(depot.value) != JSON.stringify(depotReset.value)
            ) {
